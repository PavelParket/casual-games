...
export function useWebSocket<T extends WSMessage = WSMessage>(
   ...
   const connect = useCallback((url: string) => {
      ...
      console.debug(`[WS] connect (generation=${generation}, isReconnect=${isReconnect})`);
      ...
      socket.onopen = () => {
         if (generation !== generationRef.current) {
            console.debug(`[WS] onopen ignored — generation mismatch (${generation} vs ${generationRef.current})`);
            return;
         }
         ...
         console.debug(`[WS] open (wasReconnect=${wasReconnect})`);

         startHeartbeat();

         if (wasReconnect) {
            console.info("[WS] reconnect succeeded");
      ...
      socket.onmessage = (event) => {
         if (generation !== generationRef.current) {
            console.debug(`[WS] onmessage ignored — generation mismatch`);
            return;
         }
         ...
         try {
            ...
            console.debug(`[WS] message received: event=${data.event}`);
      ...
      socket.onclose = (event) => {
         if (generation !== generationRef.current) {
            console.debug(`[WS] onclose ignored — generation mismatch`);
            return;
         }
         if (isUnmounting.current) {
            return;
         ...
         console.debug(`[WS] close: code=${event.code} wasEverOpen=${wasEverOpenRef.current} → ${action}`);
         ...
         if (!reconnectUsedRef.current) {
            console.info(`[WS] reconnect started (code=${event.code})`);
   ...
   useEffect(() => {
      ...
      const handleVisibilityChange = () => {
         ...
         if (isSocketDead && !isUnmounting.current && wsUrlRef.current) {
            console.info("[WS] visibility=visible, socket dead — reconnecting");
            reconnectUsedRef.current = false;
            clearReconnectTimer();
