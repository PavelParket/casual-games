...
import { MahjongRoomApi, RoomAPI } from "../../api/WsHubApi";
import type { RootState } from "../store";

export const MAHJONG_OPERATION_KEYS = {
    GET_ROOM: "getRoom",
...
export const getPlayersBets = createAsyncThunk<PlayerBet[], { roomId: string }, { rejectValue: string }>(
    "mahjongRoom/getPlayersBets",
    async ({ roomId }, { rejectWithValue }) => {
        try {
            const response = await MahjongRoomApi.getPlayersBets(roomId);
            return response.data;
        } catch (err: unknown) {
...
const initialState: MahjongRoomState = {
    room: undefined,
...
const mahjongRoomSlice = createSlice({
    name: "mahjongRoom",
    initialState,
    reducers: {
        ...
        clearMahjongRoomState: () => initialState,
    },
    extraReducers: (builder) => {
        builder
            /* === Get Room === */
            .addCase(getRoomById.pending, (state) => {
                state.errors[MAHJONG_OPERATION_KEYS.GET_ROOM] = null;
            })
            .addCase(getRoomById.fulfilled, (state, action) => {
                state.room = action.payload;
            })
            .addCase(getRoomById.rejected, (state, action) => {
                state.errors[MAHJONG_OPERATION_KEYS.GET_ROOM] = action.payload?.message ?? "Failed to fetch room";
            })

            /* === Room Status === */
            .addCase(getRoomStatus.pending, (state) => {
                state.errors[MAHJONG_OPERATION_KEYS.GET_ROOM_STATUS] = null;
            })
            .addCase(getRoomStatus.fulfilled, (state, action) => {
                state.roomStatus = action.payload;
            })
            .addCase(getRoomStatus.rejected, (state, action) => {
                state.errors[MAHJONG_OPERATION_KEYS.GET_ROOM_STATUS] = action.payload ?? "Failed to fetch room status";
            })

            /* === Players === */
            .addCase(getPlayers.pending, (state) => {
                state.errors[MAHJONG_OPERATION_KEYS.GET_PLAYERS] = null;
            })
            ...
            .addCase(getPlayers.rejected, (state, action) => {
                state.errors[MAHJONG_OPERATION_KEYS.GET_PLAYERS] = action.payload ?? "Failed to fetch players";
            })

            /* === Ready Players === */
            .addCase(getReadyPlayers.pending, (state) => {
                state.errors[MAHJONG_OPERATION_KEYS.GET_READY_PLAYERS] = null;
            })
            ...
            .addCase(getReadyPlayers.rejected, (state, action) => {
                state.errors[MAHJONG_OPERATION_KEYS.GET_READY_PLAYERS] = action.payload ?? "Failed to fetch ready players";
            })

            /* === Player Bets === */
            .addCase(getPlayersBets.pending, (state) => {
                state.errors[MAHJONG_OPERATION_KEYS.GET_PLAYERS_BETS] = null;
            })
            ...
            .addCase(getPlayersBets.rejected, (state, action) => {
                state.errors[MAHJONG_OPERATION_KEYS.GET_PLAYERS_BETS] = action.payload ?? "Failed to fetch player bets";
            });
    },
});

export const { clearError, clearAllErrors, clearMahjongRoomState } = mahjongRoomSlice.actions;

export default mahjongRoomSlice.reducer;
