...
@Component
@Slf4j
public class MahjongBusinessValidator implements GameBusinessValidator<MahjongTransactionRequest> {
    ...
    @Override
    public void validate(MahjongTransactionRequest request) {
    ...
    private void validateEqualBets(List<PlayerBetMessage> bets) {
        if (bets.size() != 2) {
            throw new BadRequestException(String.format(INVALID_PLAYERS_COUNT, "Mahjong"));
        }
